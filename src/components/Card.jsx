...
export default function Card({ img, title, description, link, priority = false }) {
  ...
  return (
    <section
      className="card group relative flex flex-col w-full max-w-[320px] h-fit p-3 pb-[18px] overflow-hidden rounded-2xl border border-solid border-[#3f3f3fa2] bg-linear-to-br from-[#232323] to-[#0f0f0f] select-none"
    >
      {/* Image container (inset by parent padding, with rounded corners) */}
      <div className="relative w-full h-[180px] bg-[#202225] rounded-xl overflow-hidden shrink-0">
        <Image
      ...
      <div className="mt-[28px] px-1 relative flex-1">
        <div className="pr-6">
          ...
          <p className="text-[#7F8080] text-sm mt-0 line-clamp-2 leading-relaxed">
            {description}
          </p>
        </div>
